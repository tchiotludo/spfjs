 ...
 * @param {XMLHttpRequest=} xhr The XMLHttpRequest for current error
 * @private
 */
spf.nav.handleNavigateError_ = function(options, url, err, xhr) {
  ...
  // reload the page.
  if (!spf.nav.dispatchError_(url, err, options, undefined, xhr)) {
    return;
  }
 ...
 * @private
 */
spf.nav.dispatchError_ = function(url, err, opt_options, opt_noEvents, xhr) {
  var detail = {'url': url, 'err': err, 'xhr': xhr};
